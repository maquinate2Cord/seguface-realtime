...
type Row = { userId:string; score:number; events:number };

export default function TopOffendersTable({ rows, limit=10 }:{ rows: Row[]; limit?: number }) {
  const sorted = [...rows].sort((a,b)=>{
    const sa = a.score ?? 0, sb = b.score ?? 0;
    if (sa !== sb) return sa - sb;              // peor score primero
    return (b.events ?? 0) - (a.events ?? 0);   // luego más eventos
  }).slice(0, limit);

  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4">
      <div className="text-sm font-semibold mb-2">Top Offenders</div>
